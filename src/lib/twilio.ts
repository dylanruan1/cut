...
export async function sendSms(
  ...
  // Someone who replied STOP must not be messaged again. Twilio blocks these
  // at its edge anyway, but checking here means Cut stops pretending it sent
  // something — and stops writing SmsLog rows for messages nobody received.
  try {
    ...
    if (optedOut) {
      console.warn("[twilio] SMS skipped — recipient opted out", {
        to: normalisedTo,
        type,
      });
    ...
    // A failed opt-out lookup must not block a booking confirmation. Twilio
    // still enforces the opt-out, so the worst case is a wasted API call.
